...
export function GoogleTagManager() {
  ...
  return (
    <>
      {/* Consent Mode v2 DEFAULT — must run BEFORE GTM loads, so every tag that
          reads consent sees DENIED until the KVKK banner (CookieConsent) flips it.
          `beforeInteractive` orders it ahead of the `afterInteractive` GTM loader
          below. It also exposes `window.gtag`, which the banner calls to `update`. */}
