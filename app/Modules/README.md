...
| **Offer** | What makes the catalog sellable: a seller org's price + stock for one variant, its lifecycle, the computed buy box, the public "product + its offers" surface, the storefront product-listing contributor (ADR-042–046), and the **seller offer feed** — a token-authed API + seller CSV that drive the offer actions so stock still reaches Inventory and search (ADR-076). **No cart, order, payment or commission** — those are later sprints — *complete, not frozen* | [modules/Offer.md](../../docs/modules/Offer.md) |
...
| **Questions** | "Satıcıya Sor" — a signed-in shopper's public question about a product, aimed at the **buy-box seller** the server derives and snapshots, answered by that seller from their panel (ADR-070–071). **No purchase gate** (the mirror of Reviews) and **reactive** moderation: the answer publishes it, an admin hides after the fact and never answers. **No photos, no rating, no thread** — *complete, not frozen* | [modules/Questions.md](../../docs/modules/Questions.md) |
| **Loyalty** | Customer points: earning (signup, a published review, a purchase whose return window closed), an **append-only ledger whose balance is a SUM** — no `balance` column — the admin "Puan Ayarları" rates screen and the customer read API (ADR-081–083). Imports no module: Identity and Reviews arrive by class-string, Order and the review author through Core. **Redemption at checkout is Phase 2 (ADR-084) and is not built** — *Phase 1 built* | [modules/Loyalty.md](../../docs/modules/Loyalty.md) |
| **Marketing** | Outbound ad-platform conversions: a server-side Meta **Conversions API** `Purchase` for every paid checkout group, deduplicated with the browser pixel by `event_id` = payment uuid, because the consent-gated pixel sees almost no purchases. Hears `PaymentSucceeded` by class-string, reads the basket through `OrderQueryContract`, queued, and can never fail a payment. **Imports no module** — *built* | [modules/Marketing.md](../../docs/modules/Marketing.md) |
...
| **Questions** → *nothing* | — | Built to Reviews' SHAPE and importing none of it — "mirrors" describes the design, never a dependency. The buy-box seller it targets arrives through `OfferQueryContract`, the product through `CatalogBrowseContract`, the seller's tenancy through Organization + Store contracts. |
