...
/**
 * What an offer's uuids are CALLED — resolved once per request.
 *
 * An offer stores `product_uuid` and `variant_uuid` and nothing else about the
 * thing it prices (ADR-040). That is the right storage decision and a terrible
 * one to render directly: a seller looking at their listings needs "Pamuklu
 * Tişört — Kırmızı / M", not two uuids. The alternative — copying the title onto
 * the offer row — is precisely what ADR-037 refuses, because a title edited in
 * the catalog would then disagree with every seller's stale copy, forever.
 *
