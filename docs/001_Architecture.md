...
| 27 | The seller offer feed gains its READ half — `GET /api/v1/seller/offers` | **Applied (2026-09-08, extends ADR-076).** The feed could write and not ask, so an integration knew what it had pushed and nothing about what the platform holds — which barcodes matched, which offers an admin suspended, what stock is believed to be on the shelf. The list answers in the feed's own vocabulary: **keyed by `gtin`**, not by a uuid the seller's ERP has never seen, with `status` and `in_stock` filters and a page size clamped at 200. **The merchant is resolved from the token and there is no `org` parameter**, so the query cannot express another shop and needs no policy call to refuse one — the write half's model, unchanged. A row whose catalogue entry lost its barcode still lists, with `gtin: null`, because hiding it would make the list disagree with the seller's own books. Offer still imports no module: titles and SKUs come from `CatalogBrowseContract`, primed once per page, and **`variantSummaries()` gained `gtin`** — a read-only Core-contract addition on the same footing as the two methods the buy box asked of Store. |
| 26 | ADR-074 amended — the bulk import INSERTS and never edits, and the admin panel gains the correction form that replaces it | **Applied (2026-09-08, owner's call).** A matching GTIN used to overwrite title, description, category, brand and KDV from the sheet; because the description cell becomes `''` when the column is absent, a supplier file without one **blanked 1,941 descriptions in a single run**, 477 of them approved copy that had to be re-imported. It now throws `gtinAlreadyInCatalog` — a skipped row carrying a Turkish sentence into `failed_import_rows` — so re-uploading a full file adds what is new and touches nothing else. The convenience it removes is real and stated: no correction pass, and no topping up a product's missing photos from a later file. Both land on `ProductModerationResource`, whose `canEdit()` now defers to `ProductPolicy` (which always allowed a moderator) and whose new edit page drives `UpdateProductAction` — **slug and GTIN deliberately absent from the DTO**, so a corrected title cannot move an indexed URL and cannot detach the product from either importer's match key. For a SELLER's proposal, sending it back with a reason is still the designed path. |
| 25 | ADR-060 amended — the buyer GOES TO PayTR's page, because the embedded frame never opened 3-D Secure | **Applied (2026-09-07).** Eleven live attempts, zero completions: PayTR's own callbacks said "customer left the payment page" ten times and "customer did not complete 3-D Secure" once, while every server-side step answered `200` and a freshly minted token read back healthy (merchant live, ok/fail URLs correct). The owner opened the SAME token in a plain tab and reached the bank's 3DS screen — so the frame was the fault. Inside our document PayTR is a **third-party context** and a browser restricting third-party storage kills the card submission silently; the session is also **single-use** (`get/token/{token}` → `200` once, then `410 Gone`), so any remount, refresh or back button inside a frame serves a dead page. `PayTrCheckout` now navigates the top window to `https://www.paytr.com/odeme/guvenli/{token}` and keeps a manual link for a browser that blocks it. **ADR-060's guarantee is untouched** — card, CVV and 3DS still never reach this app, and the server-to-server callback is still the truth. Cost: the buyer visibly leaves the domain for the card step, and the return now rides on `merchant_ok_url`/`merchant_fail_url`. **Still open:** `order.payment_window_minutes` is 5 while PayTR's session is 30, so a slow 3DS still expires the order mid-payment (ADR-072 then re-reserves or refunds) — an operator can lengthen it without a release. |
| 25 | ADR-091 — a server-side render is first-party traffic, and the storefront rate limiter counts outsiders | **Accepted (2026-09-04, implemented 2026-09-03 as an outage fix).** `/urunler` answered "a server-side exception has occurred" only under load while thirty concurrent `GET /api/v1/products` all returned 200 — the digest resolved in the Next log to **11,094 × `POST /api/v1/offers/prices` → 429**. It was our own limiter: the storefront renders on the same box and fetches over loopback, so **every shopper's render was 127.0.0.1** sharing one 300/minute bucket, and a listing spends two calls per render. (The home page swallows fetch errors into an empty strip; the listing threw — which is why one page turned a 429 into a 500.) Renders are now exempt, keyed on a **CGI parameter the loopback-only vhost sets** (`fastcgi_param INTERNAL_RENDER 1`) — **not** the socket address (every request reaches FPM over loopback, so `REMOTE_ADDR` cannot tell a render from a visitor) and **not** a header (a client's arrives as `HTTP_INTERNAL_RENDER`; a test pins that), and not a shared secret, since the network already draws the boundary and the variable only reports which side a request came from. No route becomes unthrottled — one class of client stops being counted. Cost: a runaway SSR loop is no longer bounded here (our bug, our box, visible as load); the protection now depends on **nginx config outside this repository** in both directions; and 300/minute has never been measured against browser traffic alone. Deferred: caching the buy-box prices per page (ADR-079's shape) is the real answer to the page's cost. Open follow-up: assert the loopback path still carries the marker, so a rebuilt vhost fails loudly instead of in an outage. Work order: `BUILD_LISTING_500_ROOTCAUSE.md`. |
| 24 | ADR-090 — free-text search runs on Meilisearch through Scout, with the tier-1 fold as its fallback | **Accepted (2026-08-27).** The ADR-089 fold made `gunes` find `güneş`, but a folded `LIKE` is a substring filter, not search: `seurm`, `uriaj` and `depidem` returned nothing, a brand query buried its own exact match, and there was no as-you-type surface. Meilisearch (self-hosted, loopback, one process shared by staging and prod via `SCOUT_PREFIX`) now ranks the `q` path; the Offer-aware listing still applies the sellable wall, category, brand and price and **preserves the engine's order** unless the shopper sorts by price. **Price and stock are never indexed** — the one custom ranking rule is Catalog's own `is_sellable` (ADR-079). **`null` means NO ENGINE and falls back to the fold; `[]` means no results** — search degrades, never disappears, and every fallback logs a warning while `GET /health` reports `search: up|down|disabled`. Synonyms and typo thresholds are version-controlled config pushed by `search:sync-settings`; `GET /search/suggest` feeds the storefront's drop-down (the UI is a separate storefront job). Cost: a second datastore that drifts, eventual consistency behind the `search` queue (**a worker is part of the feature**), editorial synonyms that go stale quietly, and lexical — not semantic — matching. Deliberately not built: a sales-count ranking rule (that number is Order's and Catalog has no synced copy), and the seller picker stays on the fold. Work order: `BUILD_SEARCH_MEILISEARCH.md`. |
| 23 | ADR-089 — transactional mail leaves over HTTP, never SMTP, behind a Resend→SES failover chain | **Accepted (2026-08-24).** AWS REFUSED SES production access, so that account reaches only verified addresses and no real customer can reset a password. **The constraint is the host:** outbound SMTP is blocked upstream (25/465/587/2465/2587 time out to three providers; 22/53/443 open, `ufw` ACCEPT on output, no local rules), so any fallback must also be an HTTP API — `resend/resend-laravel` added, `MAIL_MAILER=failover` with `['resend','ses']`. **`smtp` removed from the chain as a FIX, not tidying:** failover tries each mailer in turn, so a dead port burned a full connection timeout per message before reaching one that works. SES stays second — free while it cannot deliver, a real second line the day it is approved. The key resolves under BOTH `RESEND_API_KEY` (the package's own config) and `RESEND_KEY` (Laravel's `services.resend.key`), so either set of docs works. Cost: two DNS identities to keep valid (a DKIM lapse fails into spam, silently); deliverability now depends on an outside vendor; and failover covers only TRANSPORT failures — a message Resend accepts and later bounces never reaches SES. Suppression/bounce webhooks not built. **Inert until the owner sets `RESEND_API_KEY` on the server and adds Resend's DKIM records in Cloudflare** — until then mail queues and fails visibly in `failed_jobs`. **Live on 2026-08-24**: both owner steps landed, prod switched to `failover`, a real registration and password reset delivered, and the six `VerifyEmailNotification`s SES had refused with `403` on 20–21 August retried to zero (their signed links are built when the job runs, so they were not stale). One failure mode found in the doing: a **missing** key does not fail over — `ApiKeyIsMissing` is an `InvalidArgumentException`, which Symfony's failover transport does not catch, so `failover` without the key is worse than `ses`. **Hardened 2026-08-24 (`BUILD_MAIL_RESILIENCE.md`)**: `retry_after` is a BLAST RADIUS, not a backoff — a rejected `@example.com` recipient marked both transports dead and the next real notifications died with "No transports found", so `BlockedRecipientGuard` now drops undeliverable recipients before any transport is asked (removing recipients, never throwing) and the window drops 60 → 5s. `users:purge-test-accounts` clears the accounts that pull the trigger, keeping any with orders, payments, points, reviews, questions or returns. Work order: `BUILD_MAIL_FALLBACK.md`. |
| 22 | ADR-088 — product descriptions are generated from the product's own fields, deterministically, and claim nothing | **Accepted (2026-08-24).** The catalogue came from a supplier file with no description column (ADR-074), so every sellable product had an empty one and the whole catalogue was excluded from the Merchant feed (ADR-086). `catalog:fill-descriptions` assembles one per product from its own title, brand, category and tokens parsed from that title. **Deterministic, not LLM**: nothing is inferred, so a title that states no volume yields a sentence that mentions none. **It claims nothing** — Turkish law, not taste — and the forbidden-claim patterns are PHRASE-shaped, because the mandatory supplement footer contains "hastalıkların *tedavisinde kullanılmaz*" and a bare-word scan would fail the build on the sentence the regulation requires. **Drives `UpdateProductAction`, writes no model** (ADR-074/076's rule): a query-builder update fires no model events, so Scout never hears and the row goes stale in search. **Only empty, never overwritten** — which makes it idempotent for free. **A department that mixes regulations is not a family**: `anne-ve-bebek` (baby shampoo AND infant formula) is left unmapped after it produced "Devam Sütü … Haricen kullanım içindir" on live data. Three live-data corrections recorded: `mg` is a dose not a net quantity; a `12 x 5 ml` multiplier means the parsed figure is not the total; Turkish agglutinates so form words need a CLOSED suffix set (an open one matches `jelatin` as `jel`). Two deviations: scope is every PUBLISHED product (`is_sellable` is a 10-minute cache — scoping to it makes the result depend on when it ran), and a missing brand does not skip the row (3,359 of 7,025 have none, and the sentence is true without one). Cost: template prose, so this is feed eligibility and an honest floor, not content that ranks — organic weight comes from FAQs and reviews; and no marker distinguishes generated text from written text for a future overwrite. Result: 19,886 descriptions in under three minutes, feed 0 → 6,967 items. Work order: `BUILD_PRODUCT_DESCRIPTION_TEMPLATE.md`. |
| 21 | ADR-087 — the review invitation is a nightly sweep, asked once per delivered line, and an opt-out is recorded rather than skipped | **Accepted (2026-08-22).** Reviews exist but nothing asks for them, so `aggregateRating` stays empty and Google shows no stars. A daily `reviews:request-pending` emails a buyer once, `settings('reviews.request_delay_days')` (default 3) after delivery. **A sweep, not a delayed job**: a three-day job delay puts the whole review funnel inside a queue where a restarted worker loses it silently — the ADR-072 shape for the ADR-072 reason. **Idempotent twice over** on `review_requests.order_line_uuid` (checked *and* UNIQUE), because Order re-offers every delivered line nightly by design — a reader that filtered on Reviews' table would be Order reaching into Reviews. **An opt-out is RECORDED, not skipped** (`sent_at` null + `suppressed_reason`): skipping silently would re-evaluate the same declining customer forever and leave nobody able to say what opt-out costs the funnel. **Reviews imports no module** — delivered lines via a new `OrderQueryContract::deliveredLinesForReviewInvitation()` (Order reads the date through `ShipmentQueryContract`, as it already does for points), "already reviewed" from its own table, recipient an `App\Models\Customer`. **Work-order correction recorded:** the brief's "new `NotificationType::ReviewRequested`" cannot be — that enum is the CHANNEL enum (ADR-006); it is a `ReviewRequestedNotification` on `NotificationType::Mail`. Cost: one invitation, no reminder; suppressed buyers are never re-asked; inert without the scheduler (the quietest such failure — an absence of reviews looks exactly like customers not bothering); and gated on SES production access. Work order: `BUILD_REVIEW_REQUEST.md`. |
